  ...
  if (COACH_SHOW_HELP) {
    host.innerHTML = `
      ...
      <div class="helperPanel">
        ...
        <ul class="helperList">
          <li>The current Essential 55 weekly intro opens on load, then continues to conjugation practice setup.</li>
    ...
    host.innerHTML = `
      ...
      <div class="helperPanel">
        ...
        <ul class="helperList">
          ...
          <li>Use conjugation practice for scored testing and the main table for reference.</li>
        </ul>
      </div>
...
function getPracticeVerbs() {
  const keys = PRACTICE_STATE.verbKeys?.length ? PRACTICE_STATE.verbKeys : [PRACTICE_STATE.verbKey];
  return keys.map(findVerbByKey).filter(Boolean);
}

function defaultPracticeExtraSlots(primaryVerb) {
  const regularPatterns = ["regular-ar", "regular-er", "regular-ir"];
  const primaryPattern = inferPatternCategory(primaryVerb);
  const preferred = regularPatterns.filter(pattern => pattern !== primaryPattern);
  const order = preferred.concat(regularPatterns.filter(pattern => !preferred.includes(pattern)));
  return [0, 1].map(idx => ({
    enabled: false,
    query: "",
...
function ensurePracticeSetupSlots(primaryVerb) {
  if (
    PRACTICE_STATE.setupBaseVerbKey === primaryVerb._key &&
    Array.isArray(PRACTICE_STATE.extraVerbSlots) &&
    PRACTICE_STATE.extraVerbSlots.length === 2
  ) {
    return;
  }
  PRACTICE_STATE.setupBaseVerbKey = primaryVerb._key;
  PRACTICE_STATE.extraVerbSlots = defaultPracticeExtraSlots(primaryVerb);
}

function renderPracticePatternOptions(value) {
  const options = [
    ["all", "Any pattern"],
...
function renderPracticeTagOptions(value) {
  const options = [
    ["all", "Any set"],
    ["core501", "501 source"],
    ["essential55", "55 Essential"],
...
function renderPracticeVerbDatalist() {
  const options = getAllVerbs()
    .filter(hasPracticeAnswerKey)
    .map(verb => `<option value="${escapeHtml(verb.infinitive)}" label="#${getDisplayVerbNumber(verb)} ${escapeHtml(verb.meaning_en || "")}"></option>`)
    .join("");
  return `<datalist id="practiceVerbChoices">${options}</datalist>`;
}

function renderPracticeExtraVerbControls(primaryVerb) {
  ensurePracticeSetupSlots(primaryVerb);
  const slots = PRACTICE_STATE.extraVerbSlots;
  return `
    <div class="practicePanel">
      <div class="practiceSectionTitle">Verbs</div>
      <div class="practiceSetupGrid">
        <div class="practiceLockedVerb">
          <div class="practiceSetupLabel">Active verb</div>
          <div class="practiceSetupVerb">${escapeHtml(primaryVerb.infinitive)} <span class="pill">#${getDisplayVerbNumber(primaryVerb)}</span></div>
        </div>
        ${slots.map((slot, idx) => `
          <div class="practiceExtraSlot">
            <label class="practiceSlotToggle">
              <input type="checkbox" data-practice-extra-enabled="${idx}" ${slot.enabled ? "checked" : ""}>
              <span>Extra verb ${idx + 2}</span>
            </label>
            <div class="practiceSlotControls">
              <div class="practiceField">
                <label for="practiceExtraVerb${idx}">Specific verb</label>
                <input
                  id="practiceExtraVerb${idx}"
                  class="practiceTextInput"
                  type="text"
                  list="practiceVerbChoices"
                  data-practice-extra-query="${idx}"
                  value="${escapeHtml(slot.query || "")}"
                  placeholder="Random from filters"
                  autocomplete="off"
                  spellcheck="false"
                >
              </div>
              <div class="practiceField">
                <label for="practiceExtraPattern${idx}">Pattern</label>
                <select id="practiceExtraPattern${idx}" class="practiceSelect" data-practice-extra-pattern="${idx}">
                  ${renderPracticePatternOptions(slot.pattern || "all")}
                </select>
              </div>
              <div class="practiceField">
                <label for="practiceExtraTag${idx}">Set</label>
                <select id="practiceExtraTag${idx}" class="practiceSelect" data-practice-extra-tag="${idx}">
                  ${renderPracticeTagOptions(slot.tag || "all")}
                </select>
              </div>
            </div>
          </div>
      ...
      ${renderPracticeVerbDatalist()}
    </div>
  `;
}

function getPracticeExtraSlotsFromModal() {
  return [0, 1].map(idx => ({
    enabled: !!document.querySelector(`#practiceModal [data-practice-extra-enabled="${idx}"]`)?.checked,
    query: normalizeUserCellInput(document.querySelector(`#practiceModal [data-practice-extra-query="${idx}"]`)?.value || ""),
    pattern: document.querySelector(`#practiceModal [data-practice-extra-pattern="${idx}"]`)?.value || "all",
    tag: document.querySelector(`#practiceModal [data-practice-extra-tag="${idx}"]`)?.value || "all"
  }));
}

function parsePracticeVerbInput(input) {
  const cleaned = cleanText(input || "");
  if (!cleaned) return null;
  const idMatch = cleaned.match(/^#?(\d{1,6})$/);
  if (idMatch) {
...
function getPracticeCandidateVerbs(filters = {}) {
  const pattern = filters.pattern ?? PRACTICE_STATE.nextPatternFilter ?? "all";
  const tag = filters.tag ?? PRACTICE_STATE.nextTagFilter ?? "all";
  const excludeKeys = new Set(filters.excludeKeys || []);
  return getAllVerbs().filter(verb => {
    if (!hasPracticeAnswerKey(verb)) return false;
    if (excludeKeys.has(verb._key)) return false;
...
function choosePracticeCandidate(filters, excludeKeys) {
  const candidates = getPracticeCandidateVerbs({ ...filters, excludeKeys: Array.from(excludeKeys || []) });
  if (!candidates.length) return null;
  return candidates[Math.floor(Math.random() * candidates.length)];
}

function resolvePracticeVerbKeysFromSlotConfigs(baseVerbKey, slotConfigs) {
  const primaryVerb = findVerbByKey(baseVerbKey);
  if (!primaryVerb || !hasPracticeAnswerKey(primaryVerb)) return null;
  const selected = [primaryVerb];
  const usedKeys = new Set([primaryVerb._key]);

  for (const slot of slotConfigs || []) {
    if (!slot.enabled) continue;
    let verb = null;
    if (slot.query) {
      verb = parsePracticeVerbInput(slot.query);
      if (!verb) {
        alert(`Could not find "${slot.query}".`);
        return null;
      }
      if (!hasPracticeAnswerKey(verb)) {
        alert(`No answer key is available for ${verb.infinitive} yet.`);
        return null;
      }
      if (usedKeys.has(verb._key)) {
        alert(`${verb.infinitive} is already in this practice set.`);
        return null;
      }
    } else {
      verb = choosePracticeCandidate({ pattern: slot.pattern || "all", tag: slot.tag || "all" }, usedKeys);
      if (!verb) {
        alert("No extra verb matches those filters.");
        return null;
      }
    }
    selected.push(verb);
    usedKeys.add(verb._key);
  }
...
function renderPracticeSetup(verbKey = CURRENT_VERB_KEY, options = {}) {
  ...
  `;
  showPracticeModal("Conjugation practice setup", `${setupVerb.infinitive} #${getDisplayVerbNumber(setupVerb)}`, body);
}
...
function renderPracticeIntro(verbKey, selectedKeys, verbKeys = null, options = {}) {
  ...
  const title = challenge ? `${getPracticeChallengeLabel(challenge)} intro` : "Conjugation practice intro";
...
function renderCombinedLeaderboardTable(leaderboard) {
  ...
  const rows = entries.map((entry, idx) => {
    ...
    const actionLabel = challenge ? (challenge.label || getPracticeChallengeLabel(challenge)) : "Conjugation practice";
    const actionTitle = challenge?.focus || "Practice this conjugation set";
    return `
    <tr>
...
function renderCombinedLeaderboardView() {
  const body = `
    ...
    <div class="practiceActions">
      ...
      <button type="button" class="practiceActionBtn" data-practice-setup>Conjugation practice</button>
    </div>
  `;
  ...
  return `
    <div class="practiceInputWrap">
      <input
        ...
        type="text"
        data-practice-input-key="${escapeHtml(inputKey)}"
        data-practice-verb-key="${escapeHtml(verb._key)}"
...
function getPracticeSelectedTenseDescriptors(verbs, sourceName, selectedKeys) {
  ...
  verbs.forEach(verb => {
    getOrderedTenseKeys(verb[sourceName] || {}).forEach(key => {
      const num = extractTenseNumber(key);
      if (!selected.has(String(num)) || byNumber.has(num)) return;
      byNumber.set(num, { num, label: key.replace(/^\d+\s*/, "") });
    });
  });
  return Array.from(byNumber.values()).sort((a, b) => a.num - b.num);
}

function buildPracticeRows(verbs, selectedKeys) {
  const selected = new Set(practiceSelectionKeys(selectedKeys));
  const rows = [];
  const hasExpected = cellKey => verbs.some(verb => cleanText((getExpectedMap(verb) || {})[cellKey] || ""));

  if (selected.has("gerund") && hasExpected(gerundCellKey())) {
    rows.push({ type: "cell", label: "Gerund", cellKey: gerundCellKey() });
  }
  if (selected.has("participle") && hasExpected(participleCellKey())) {
    rows.push({ type: "cell", label: "Part.", cellKey: participleCellKey() });
  }
  ...
  if (selected.has("imperative")) {
    ...
    const imperativeRows = slots
      .map(slot => ({ type: "cell", label: IMPERATIVE_META[slot].label, cellKey: imperativeCellKey(slot) }))
      .filter(row => hasExpected(row.cellKey));
    if (imperativeRows.length) {
      rows.push({ type: "section", label: "Imperative" });
      rows.push(...imperativeRows);
    }
  }
...
function renderPracticeMatrix(verbs, selectedKeys) {
  ...
  return `
    <div class="practiceMatrix" style="--practice-verb-count: ${verbs.length};">
      <div class="practiceMatrixCorner"></div>
      ${verbs.map(verb => `
        <div class="practiceMatrixVerb">
          <div>${escapeHtml(verb.infinitive)}</div>
          <span class="pill">#${getDisplayVerbNumber(verb)}</span>
        </div>
      `).join("")}
      ${rows.map(row => {
...
function summarizePracticeInputs(verbs, selectedKeys, inputs) {
  const statusByCell = {};
  ...
  verbs.forEach(verb => {
    ...
    getPracticeCellKeys(verb, selectedKeys).forEach(cellKey => {
      const inputKey = practiceInputKey(verb._key, cellKey);
      const status = compareUserToExpected(inputs[inputKey] || "", expected[cellKey] || "");
      statusByCell[inputKey] = status;
      summary[status] += 1;
      summary.total += 1;
  ...
  return { statusByCell, summary, perVerb };
}
...
const INFINITIVE_GAME_VERSION = "infinitive-game-v3";
...
const INFINITIVE_GAME_PROMPT_OVERRIDES = {
  conocer: "to know / be familiar with a person, place, or thing",
  estar: "to be (state, condition, feeling, or location)",
  haber: "to have as an auxiliary; there is / there are",
  ir: "to go",
  irse: "to leave / go away",
  llevar: "to carry, wear, or take with you",
  mirar: "to look at / watch",
  oir: "to hear",
  parecer: "to seem / appear",
  saber: "to know a fact, information, or how to do something",
  salir: "to leave / go out / come out",
  ser: "to be (identity, origin, time, or essential quality)",
  tener: "to have / possess",
  tomar: "to take, drink, or have",
  traer: "to bring toward the speaker",
  venir: "to come",
  ver: "to see"
};
const INFINITIVE_GAME_CLUE_NOTES = {
  caer: "Think of falling or dropping.",
  caerse: "Reflexive: falling down or falling over.",
  ...
  tomar: "This is taking something, drinking something, or having food/drink.",
  traer: "This is bringing something toward the speaker.",
  venir: "This is coming toward the speaker or destination.",
  ver: "This is seeing."
};
...
function formatInfinitiveGameMeaning(verb) {
  ...
  if (INFINITIVE_GAME_PROMPT_OVERRIDES[key]) return INFINITIVE_GAME_PROMPT_OVERRIDES[key];
  const info = splitMeaningAndCaution(verb?.meaning_en || "");
  return cleanText(
    info.meaning ||
    verb?.meaning_en ||
    INFINITIVE_GAME_MEANING_FALLBACKS[key] ||
    ""
  );
}
    ...
    <details class="practiceAdvancedDetails" open>
      ...
      <div class="practiceAdvancedBody">
        ...
        <div class="practiceTypingHint">
          You will see the English meaning and type the Spanish infinitive. You get three tries per verb, then the recall round moves on. The answer shape starts hidden; using reveals or clues is tracked on the leaderboard.
        </div>
        ${fullWarning}
      </div>
    </details>
...
function markInfinitiveGameHintUsed(game, type) {
  ...
  if (!Array.isArray(game.helpedVerbKeys)) game.helpedVerbKeys = [];
  const verbHelpKey = String(Number(game.currentIndex) || 0);
  if (!game.helpedVerbKeys.includes(verbHelpKey)) {
    game.helpedVerbKeys.push(verbHelpKey);
    game.helpedVerbCount = Math.max(0, Number(game.helpedVerbCount) || 0) + 1;
  }
  if (type === "shape") game.shapeRevealCount = Math.max(0, Number(game.shapeRevealCount) || 0) + 1;
  if (type === "first-letter") game.firstLetterCount = Math.max(0, Number(game.firstLetterCount) || 0) + 1;
  if (type === "clue") game.clueCount = Math.max(0, Number(game.clueCount) || 0) + 1;
  return true;
}
...
function buildGenericInfinitiveGameClue(entry) {
  const infinitive = cleanText(entry?.infinitive || "");
  const lower = infinitive.toLocaleLowerCase("es-ES");
  const reflexive = lower.endsWith("se");
  const base = reflexive ? lower.slice(0, -2) : lower;
  const ending = base.endsWith("ar") ? "-ar" : base.endsWith("er") ? "-er" : base.endsWith("ir") ? "-ir" : "";
  const parts = [];
  if (reflexive) parts.push("It is reflexive, so the infinitive ends in -se.");
  if (ending) parts.push(`It belongs to the ${ending} infinitive family.`);
  parts.push("Think of the infinitive form, not a conjugated form.");
  return parts.join(" ");
}

function getInfinitiveGameClue(entry) {
  const key = normalize(entry?.infinitive || "");
  const note = INFINITIVE_GAME_CLUE_NOTES[key] || buildGenericInfinitiveGameClue(entry);
  return note;
}
...
function resetCurrentInfinitiveGameHelp(game) {
  game.lives = INFINITIVE_GAME_LIVES;
  game.revealed = [];
  game.shapeRevealed = false;
  game.clueText = "";
}

function advanceInfinitiveGame(message = "") {
  const game = getInfinitiveGameState();
  game.currentIndex += 1;
  resetCurrentInfinitiveGameHelp(game);
  game.message = message;
  if (game.currentIndex >= (game.entries?.length || 0)) {
...
function submitInfinitiveGameGuess() {
  ...
  if (game.lives <= 0) {
    game.answers.push({
      ...
      correct: false,
      missed: true,
      wrongAttempts: INFINITIVE_GAME_LIVES
    });
    advanceInfinitiveGame(`Missed: ${entry.infinitive}. Moving on.`);
    return;
  }
...
function buildInfinitiveGameScorePayload(game) {
  ...
  return {
    ...
    shapeRevealed: Math.max(0, Number(game.shapeRevealCount) || 0) > 0,
    hintCount: Math.max(0, Number(game.hintCount) || 0),
    helpedVerbCount: Math.max(0, Number(game.helpedVerbCount) || 0),
    shapeRevealCount: Math.max(0, Number(game.shapeRevealCount) || 0),
    firstLetterCount: Math.max(0, Number(game.firstLetterCount) || 0),
    clueCount: Math.max(0, Number(game.clueCount) || 0),
...
function formatInfinitiveGameHelpSummary(entry) {
  const helped = Number(entry.helpedVerbCount) || 0;
  const hints = Number(entry.hintCount) || 0;
  if (!helped && !hints) return "None";
  return `${helped} verbs / ${hints} hints`;
}
...
function renderInfinitiveGameLeaderboardTable(leaderboard, options = {}) {
  ...
  const rows = entries.map(entry => `
    <tr class="${entry.isCurrentAttempt ? "practiceLeaderboardCurrent" : ""}">
      ...
      <td>${entry.completed ? "Finished" : "Stopped"}</td>
      <td>${escapeHtml(formatInfinitiveGameHelpSummary(entry))}</td>
...
function renderInfinitiveGameResult() {
  ...
  const helpedRetryLabel = missedCount
    ? `Recall missed + helped (${missedAndHelpedCount})`
    : `Recall helped (${missedAndHelpedCount})`;
  ...
  const body = `
    <div class="practicePanel">
      ...
      <div class="infinitiveGameResultGrid">
        ...
        <div><span>Help used</span><strong>${escapeHtml(helpSummary)}</strong></div>
      </div>
      ${failedEntry ? `
    ...
    <div class="practiceActions">
      ${missedCount ? `<button type="button" class="practiceSecondaryBtn" data-infinitive-game-missed>Recall missed (${missedCount})</button>` : ""}
...
function renderInfinitiveGameLeaderboardView(options = {}) {
  ...
  const body = `
    ...
    <div class="practiceActions">
      ...
      <button type="button" class="practiceSecondaryBtn" data-infinitive-game-setup>Recall setup</button>
...
const FLASHCARD_LEGACY_DEFAULT_TENSE_KEYS = ["1", "2", "3", "4", "5"];
const FLASHCARD_DEFAULT_TENSE_KEYS = ["1", "3", "4"];
const FLASHCARD_DECKS = {
  "essential55-verbs": {
    id: "essential55-verbs",
    listId: "essential55",
    cardType: "verb-forward",
    label: "Essential 55 · Spanish → English",
    description: "See the Spanish infinitive, then reveal its English meaning."
  },
  "essential55-reverse": {
    id: "essential55-reverse",
    listId: "essential55",
    cardType: "verb-reverse",
    label: "Essential 55 · English → Spanish",
    description: "See the English meaning, then reveal the Spanish infinitive."
  },
  "essential55-cloze": {
  ...
  "core501-verbs": {
    ...
    listId: "core501",
    cardType: "verb-forward",
    label: "Full 501 · Spanish → English",
    description: "See the Spanish infinitive, then reveal its English meaning."
  },
  "core501-reverse": {
    id: "core501-reverse",
    listId: "core501",
    cardType: "verb-reverse",
    label: "Full 501 · English → Spanish",
    description: "See the English meaning, then reveal the Spanish infinitive."
  },
  "core501-cloze": {
    id: "core501-cloze",
...
function getFlashcardProgressStore() {
  ...
  try {
    ...
    const selectedKeys = Number(parsed.version) < FLASHCARD_VERSION &&
      hasSameTenseKeySet(rawSelectedKeys, FLASHCARD_LEGACY_DEFAULT_TENSE_KEYS)
      ? [...FLASHCARD_DEFAULT_TENSE_KEYS]
      : rawSelectedKeys;
...
function getFlashcardProgressCards(playerName) {
  const store = getFlashcardProgressStore();
  const displayName = sanitizePracticePlayerName(
    playerName ||
  ...
  if (!current || typeof current !== "object") {
    ...
    if (!current.cards || typeof current.cards !== "object") current.cards = {};
  }
  return store.profiles[profileKey].cards;
}
...
function buildFlashcardDeckRefs(deckId, selectedKeys) {
  ...
  if (deck.cardType !== "cloze") {
    const reverse = deck.cardType === "verb-reverse";
    return verbs.map(verb => ({
      cardId: `${reverse ? "reverse" : "verb"}|${verb._key}`,
      verbKey: verb._key,
      tenseKey: "",
...
function getFlashcardDeckStats(refs, now = Date.now(), playerName = "") {
  return (refs || []).reduce((stats, ref) => {
    const progress = getFlashcardProgressRecord(ref.cardId, playerName);
    if (!progress?.reps) {
      stats.newCount += 1;
      return stats;
    }
...
function flashcardRecordDifficultyScore(progress) {
...
function flashcardDifficultyScore(ref, playerName = "") {
  return flashcardRecordDifficultyScore(getFlashcardProgressRecord(ref.cardId, playerName));
}

function selectFlashcardSessionRefs(refs, sessionSize, seedText, playerName = "") {
  const now = Date.now();
  const due = [];
  const unseen = [];
  const future = [];
  ...
  due.sort((a, b) => {
    const difficulty = flashcardDifficultyScore(b, playerName) - flashcardDifficultyScore(a, playerName);
    if (difficulty) return difficulty;
    return (getFlashcardProgressRecord(a.cardId, playerName)?.dueAt || 0) -
  ...
  future.sort((a, b) => {
    const dueDiff = (getFlashcardProgressRecord(a.cardId, playerName)?.dueAt || 0) -
      (getFlashcardProgressRecord(b.cardId, playerName)?.dueAt || 0);
...
function renderFlashcardSetup(options = {}) {
  ...
  const body = `
    ...
    <div class="practicePanel flashcardIntroPanel flashcardFlatPanel">
      ...
      <div class="flashcardIntroText">Rate each answer Again, Hard, Good, or Easy. Difficult cards return sooner, while well-known cards are spaced further apart.</div>
    ...
    <div class="practiceActions">
      <button type="button" class="practiceSecondaryBtn" data-flashcard-progress>Progress &amp; difficult verbs</button>
      ${stats.learnedCount ? `<button type="button" class="practiceSecondaryBtn" data-flashcard-reset>Reset this deck's progress</button>` : ""}
      <button type="button" class="practiceSecondaryBtn" data-practice-setup>Conjugation practice</button>
      <button type="button" class="practiceSecondaryBtn" data-flashcard-infinitive-game>Infinitive recall</button>
    </div>
  `;
  showPracticeModal(
...
function buildFlashcardProgressRows(refs, playerName) {
  const now = Date.now();
  const byVerb = new Map();
  (refs || []).forEach(ref => {
    const verb = findVerbByKey(ref.verbKey);
    if (!verb) return;
    if (!byVerb.has(ref.verbKey)) {
      byVerb.set(ref.verbKey, {
        verbKey: ref.verbKey,
        infinitive: cleanText(verb.infinitive || ""),
        meaning: formatInfinitiveGameMeaning(verb),
        displayNumber: getDisplayVerbNumber(verb),
    ...
    const row = byVerb.get(ref.verbKey);
    row.cardCount += 1;
    const progress = getFlashcardProgressRecord(ref.cardId, playerName);
    if (!progress?.reps) return;
    row.reviewedCardCount += 1;
    row.reviews += progress.reps;
    row.lapses += progress.lapses;
    row.dueCount += progress.dueAt <= now ? 1 : 0;
    row.easeTotal += progress.ease;
    row.intervalTotal += progress.intervalDays;
    row.difficultyTotal += flashcardRecordDifficultyScore(progress);
    if (progress.lastReviewedAt >= row.lastReviewedAt) {
      row.lastReviewedAt = progress.lastReviewedAt;
      row.lastRating = progress.lastRating;
    }
  });
  return Array.from(byVerb.values()).map(row => ({
    ...row,
    difficultyScore: row.reviewedCardCount
      ? Math.min(100, Math.round((row.difficultyTotal / row.reviewedCardCount) * 12))
      : null,
    averageEase: row.reviewedCardCount ? row.easeTotal / row.reviewedCardCount : 0,
    averageInterval: row.reviewedCardCount ? row.intervalTotal / row.reviewedCardCount : 0
  }));
}
...
function flashcardDifficultyLabel(row) {
  if (!row.reviewedCardCount) return { label: "New", level: "new" };
  const score = Math.max(0, Number(row.difficultyScore) || 0);
  if (score >= 60) return { label: `High · ${score}`, level: "high" };
  if (score >= 25) return { label: `Medium · ${score}`, level: "medium" };
...
function renderFlashcardProgressTable(rows) {
  const body = (rows || []).map((row, index) => {
    ...
    const lastRating = row.lastRating
      ? `${row.lastRating.charAt(0).toUpperCase()}${row.lastRating.slice(1)}`
      : "—";
    return `
      <tr>
        <td>${index + 1}</td>
        <td>
          <strong>${escapeHtml(row.infinitive)}</strong>
          <small>#${escapeHtml(row.displayNumber)}</small>
        </td>
        <td>${escapeHtml(row.meaning)}</td>
        <td>${row.reviewedCardCount}/${row.cardCount}</td>
        <td>${row.reviews}</td>
        <td>${row.lapses}</td>
        <td>${row.dueCount}</td>
        <td><span class="flashcardDifficulty flashcardDifficulty--${difficulty.level}">${escapeHtml(difficulty.label)}</span></td>
        <td>${escapeHtml(lastRating)}</td>
      </tr>
    `;
  }).join("");
  return `
    <div class="flashcardProgressTableWrap">
...
function renderFlashcardProgressView(options = {}) {
  const current = getFlashcardState();
  const deckId = getFlashcardDeck(options.deckId || current.deckId).id;
  const deck = getFlashcardDeck(deckId);
  const selectedKeys = sanitizeFlashcardTenseKeys(options.selectedKeys || current.selectedKeys);
  ...
  const difficultVerbCount = rows.filter(row =>
    row.reviewedCardCount > 0 && (Number(row.difficultyScore) || 0) >= 25
  ).length;
  const tenseNote = deck.cardType === "cloze"
    ? ` · ${selectedKeys.map(key => TENSE_SELECTION_LABELS[key] || key).join(", ")}`
    : "";
  const body = `
    <div class="practicePanel flashcardFlatPanel">
      <div class="practiceSectionTitle">Profile overview</div>
      <div class="flashcardIntroText">Difficulty combines lapses, recent Again or Hard ratings, and reduced spacing. New verbs are listed separately from learned difficulty.</div>
      <div class="flashcardStatsGrid">
        <div><span>Verbs</span><strong>${rows.length}</strong></div>
        <div><span>Cards learned</span><strong>${stats.learnedCount}</strong></div>
        <div><span>Cards due</span><strong>${stats.dueCount}</strong></div>
...
function renderFlashcardRun() {
  ...
  const progress = getFlashcardProgressRecord(card.cardId, game.playerName);
  ...
  const front = card.cardType === "verb-forward" ? `
    ...
    ` : ""}
  ` : card.cardType === "verb-reverse" ? `
    <div class="flashcardPromptLabel">English meaning</div>
    <div class="flashcardInfinitive flashcardMeaningPrompt">${escapeHtml(card.meaning)}</div>
    <div class="flashcardInstruction">Recall the Spanish infinitive, then show the answer.</div>
    ${game.revealed ? `
      <div class="flashcardAnswer">
        <span>Spanish infinitive</span>
        <strong>${escapeHtml(card.infinitive)}</strong>
      </div>
    ` : ""}
  ` : `
...
function renderFlashcardResult() {
  ...
  const stats = getFlashcardDeckStats(refs, Date.now(), game.playerName);
  ...
  const body = `
    ...
    <div class="practiceActions">
      ...
      <button type="button" class="practiceSecondaryBtn" data-flashcard-progress>View progress</button>
...
function renderPracticeNextControls() {
  return `
    <div class="practicePanel">
      <div class="practiceSectionTitle">Next practice</div>
      <div class="practiceNextControls">
        <div class="practiceField">
          ...
          <select id="practiceNextPattern" class="practiceSelect" data-practice-next-pattern>
            ${renderPracticePatternOptions(PRACTICE_STATE.nextPatternFilter || "all")}
          </select>
        </div>
        <div class="practiceField">
          <label for="practiceNextTag">Set</label>
          <select id="practiceNextTag" class="practiceSelect" data-practice-next-tag>
            ${renderPracticeTagOptions(PRACTICE_STATE.nextTagFilter || "all")}
          </select>
        </div>
        <button type="button" class="practiceActionBtn" data-practice-random-next>Random next set</button>
      </div>
    </div>
  `;
}
...
function renderPracticeRun() {
  ...
  const submittedControls = PRACTICE_STATE.submitted
    ...
    `;
  const title = verbs.length === 1 ? "Conjugation practice" : `${verbs.length}-verb conjugation practice`;
...
function startPracticeAttempt(verbKey, selectedKeys, verbKeys = null) {
  const keys = practiceSelectionKeys(selectedKeys);
  const resolvedVerbKeys = Array.isArray(verbKeys) && verbKeys.length ? verbKeys : [verbKey];
  const verbs = resolvedVerbKeys.map(findVerbByKey).filter(Boolean);
  if (!verbs.length || verbs.some(verb => !hasPracticeAnswerKey(verb))) return;
...
function submitPracticeAttempt() {
  ...
  PRACTICE_STATE.attempts.push({
    ...
    pattern: verbs.map(inferPatternCategory).join(" / "),
    selectedKeys: [...PRACTICE_STATE.selectedKeys],
...
function chooseRandomPracticeVerb() {
  const excludeKeys = PRACTICE_STATE.verbKeys?.length ? PRACTICE_STATE.verbKeys : [PRACTICE_STATE.verbKey];
  const candidates = getPracticeCandidateVerbs({ excludeKeys });
...
function startRandomPracticeVerb() {
  ...
  if (!next) return;
  const verbKeys = resolvePracticeVerbKeysFromSlotConfigs(next._key, PRACTICE_STATE.extraVerbSlots || []);
  if (!verbKeys) return;
  selectVerbByKey(next._key);
  startPracticeAttempt(next._key, PRACTICE_STATE.selectedKeys, verbKeys);
}
...
function renderPracticeSummary() {
  ...
  const rows = PRACTICE_STATE.attempts.map((attempt, idx) => {
    ...
    const perVerb = Array.isArray(attempt.perVerb) && attempt.perVerb.length
      ? attempt.perVerb
      : [{
          infinitive: attempt.infinitive,
          displayNumber: attempt.displayNumber,
          pattern: attempt.pattern
        }];
    const verbsHtml = perVerb.map(item => (
      `<div>${escapeHtml(item.infinitive || "")} <span class="pill">#${escapeHtml(item.displayNumber || "")}</span></div>`
    )).join("");
    const patternsHtml = perVerb.map(item => (
      `<div>${escapeHtml(formatPracticePatternCategory(item.pattern || ""))}</div>`
    )).join("");
    return `
      <tr>
        <td>${idx + 1}</td>
  ...
  const body = `
    ...
    <div class="practicePanel">
      <div class="practiceSectionTitle">Practice sets</div>
      ${rows ? `
  ...
  `;
  showPracticeModal("Conjugation practice summary", `${PRACTICE_STATE.attempts.length} attempted`, body);
}
